...
class RentmanFastAPI:
    ...
    def _fetch_projects_with_oas_optimizations(self, from_date=None, to_date=None) -> List[Dict]:
        ...
        try:
            ...
            # Aggiungiamo i progetti mancanti noti con chiamate dirette
                ...
                try:
                    ...
                    if response.ok:
                        ...
                        if project:
                            ...
                            logger.info(f"[FETCH] Aggiunto progetto mancante ID {project_id} (numero {project.get('number')})")
                    else:
                        logger.warning(f"[FETCH] Progetto {project_id} non trovato: {response.status_code}")
                        
                except Exception as e:
                    logger.warning(f"[FETCH] Errore recupero progetto {project_id}: {e}")
            
            all_projects = standard_projects + additional_projects
            logger.info(f"[FETCH] Progetti totali: {len(all_projects)} (standard: {len(standard_projects)}, aggiunti: {len(additional_projects)})")
    ...
    def _filter_and_enrich_projects_oas(self, projects: List[Dict], from_date: str, to_date: str) -> List[Dict]:
        ...
        logging.info(f"[ARRICCHIMENTO] Arricchimento completato: {len(enriched_list)} progetti arricchiti")        # 3. Filtro per stati DOPO l'arricchimento - SPECIFICO per i 9 progetti Rentman
        # Stati specifici dei 9 progetti mostrati nello screenshot Rentman del 05/06/2025
        stati_validi = {
        ...
        # Filtra solo progetti con stati validi
        progetti_filtrati = []
        ...
        for p in enriched_list:
            ...
            if status in stati_validi:
                ...
                logging.info(f"[FILTRO STATI] ✅ INCLUSO - Progetto {p.get('id')} con status '{p.get('status')}'")
            else:
                logging.info(f"[FILTRO STATI] 🚫 ESCLUSO - Progetto {p.get('id')} con status '{p.get('status')}'")
